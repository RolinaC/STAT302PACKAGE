...
y_hat <- x %*% eqn$Estimate
y <- my_gapminder$lifeExp

df <- as.data.frame(cbind(y_hat, y))
ggplot2::ggplot(df, ggplot2::aes(x = y, y = y_hat)) +
  ggplot2::geom_point() +
...
Accordng to the graph, we can conclude that the model follows an excellent linear relationship since points are plotted on the best fit line.
