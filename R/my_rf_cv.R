...
my_rf_cv <- function(k) {
  ...
  mse <- c()
  for (i in 1:k) {
    training <- filter(data_cleaned, fold != i)
    testing <- filter(data_cleaned, fold == i)
    rf_m <- randomForest(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm,
                         data = training, ntree = 100)
    pred <- predict(rf_m, testing[, -1])
